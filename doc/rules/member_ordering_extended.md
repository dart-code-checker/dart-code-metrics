...
`
< (overridden | protected)- >< (private | public)- >< static- >< (var | final | const)- >< named- >< factory- > (fields | getters | getters-setters | setters | constructors | methods)
`
...
**Note:** not all of the pattern parts are applicable for every case, for example, `var-constructors` are not expected, since they are not supported by the language itself.
